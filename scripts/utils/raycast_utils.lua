...
RaycastUtils = {
    ...
    ---@param fineness integer レイキャストの判定を行う精度（1ブロックの長さにつき何回処理を行うか）。当然細かくすれば精度が向上するが、その分処理の負荷も増大する。
    getLengthBetweenPointAndCollision = function (self, worldPos, worldDir, maxLength, fineness)
        ...
        for i = 1, fineness * maxLength do
            ...
            currentPos:add(worldDir:copy():scale(1 / fineness))
            if debug then

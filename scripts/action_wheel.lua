...
events.TICK:register(function()
    ...
    if actionWheelOpened then
        ...
        if ExSkill:canPlayAnimation() and ExSkill.AnimationCount == -1 then
            mainPage:getAction(1):title(Language:getTranslate("action_wheel__main__action_1__title").."\n§b"..Language:getTranslate("action_wheel__main__action_1__title_2_"..BlueArchiveCharacter.COSTUME[Costume.CurrentCostume].ex_skill)):color(attackType == "EXPLOSION" and vectors.vec3(0.55, 0, 0.03) or (attackType == "PIERCE" and vectors.vec3(0.72, 0.55, 0.2) or (attackType == "MYSTERY" and vectors.vec3(0.16, 0.43, 0.6) or vectors.vec3(0.58, 0.28, 0.64)))):hoverColor(1, 1, 1)
        else
